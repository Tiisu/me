...
export default function WasteReportsList({ onWasteCollected, isAgent = true }) {
  ...
  // Check if reports exist and fetch them
  useEffect(() => {
    ...
    const refreshInterval = setInterval(() => {
      console.log('Auto-refreshing waste reports...');
  ...
  // Function to check if any reports exist
  const checkReportsExist = async () => {
    try {
      const result = await api.waste.checkReportsExist();
      console.log('Reports exist check:', result);

      if (result.hasReports) {
        console.log(`Database has ${result.totalReports} reports. Latest report:`, result.latestReport);
      } else {
        console.log('No reports found in the database');
      }
    } catch (error) {
      console.error('Failed to check if reports exist:', error);
    }
  };

  const fetchPendingReports = async (forceRefresh = false) => {
    try {
      setLoading(true);
      console.log('Fetching ALL waste reports' + (forceRefresh ? ' (FORCE REFRESH)' : ''));

      // Add a cache-busting parameter when force refreshing
      const params = {
        limit: 100, // Get a larger number of reports
        _t: forceRefresh ? Date.now() : undefined // Cache-busting parameter
      };

      const response = await api.waste.getWasteReports(params);
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center mt-4">
              <Button
                variant="outline"
                size="sm"
                onClick={() => fetchPendingReports(true)}
